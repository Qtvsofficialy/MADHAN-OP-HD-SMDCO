...
async def start(client, message):
    ...
    if len(message.command) != 2:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_sticker("CAACAgUAAxkBAAEC0Q1l5M_f7d9hTY6xIG9dGHo_UGlodgACaQcAAjisiVeEAfydyvbuUB4E") 
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        await client.send_photo(
            ...
            caption="**You are not in our channel given below so you don't get the movie file...\n\nIf you want the movie file, click on the '🍿ᴊᴏɪɴ ᴏᴜʀ ʙᴀᴄᴋ-ᴜᴘ ᴄʜᴀɴɴᴇʟ🍿' button below and join our back-up channel, then click on the '♣𝐓𝐫𝐲 𝐀𝐠𝐚𝐢𝐧🥀' button below...\n\nThen you will get the movie files...**",
    ...
    if data.startswith("sendfiles"):
        ...
        if not files:
            return await message.reply('<b><u>𝐍𝐨 𝐒𝐮𝐜𝐡 𝐅𝐢𝐥𝐞 𝐄𝐱𝐢𝐬𝐭</b></i>')
        filesarr = []
        for file in files:
    ...
    if not files_:
        ...
        try:
            ...
            title = '@Qtmve_linkzZz ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@Qtmve_linkzZz ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@Qtmve_linkzZz  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
        ]]
